...
export class CompetitionPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    let competitionChange$ = this.league$.map(league => this.store.state.data.competitions.find(x => x.league == league));
    ...
    let animationCompeted$ = this.nextTour$.debounceTime(200);
